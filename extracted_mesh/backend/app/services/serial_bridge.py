...
class SerialBridge:
    ...
    def _connect(self) -> bool:
        ...
        port_to_open = settings.SERIAL_PORT
        
        # --- AUTO DETECT PORT FOR LOW-TECH USERS ---
        if port_to_open.upper() == "AUTO":
            ports = list(serial.tools.list_ports.comports())
            logger.info("Auto-detecting from available ports: %s", [p.device for p in ports])
            
            # Prefer USB serial devices commonly used by LilyGO/ESP32
            valid_ports = [p.device for p in ports if "USB" in p.device.upper() or "ACM" in p.device.upper() or "COM" in p.device.upper()]
            
            if valid_ports:
                port_to_open = valid_ports[0]
                logger.info("Auto-detected serial port: %s", port_to_open)
                # Update current settings so the UI shows the real port
                settings.SERIAL_PORT = port_to_open
            else:
                logger.warning("Auto-detect failed: No compatible USB/UART serial ports found. Available: %s", [p.device for p in ports])
                self.is_connected = False
                return False

        try:
            # serial_for_url supports both physical ports like /dev/ttyUSB0 
            # and network ports like socket://192.168.4.1:8080
            ...
            logger.info("Serial port opened: %s", port_to_open)
            return True
        except serial.SerialException as e:
            self.is_connected = False
            logger.warning("Failed to open serial port %s: %s", port_to_open, e)
            return False
    ...
    def _reader_loop(self):
        ...
        while self._running:
            ...
            try:
                ...
                logger.error("Serial read error or disconnected: %s. Reconnecting...", e)
                self.is_connected = False
                if self._serial and self._serial.is_open:
                    self._serial.close()
                self._serial = None
    ...
    def _enqueue_raw(self, direction: str, data: bytes):
        """Broadcast raw bytes to WebSocket for Raw Serial Monitor."""
        if self._loop:
            self._loop.call_soon_threadsafe(self._safe_put_raw, direction, data)

    def _safe_put_raw(self, direction: str, data: bytes):
