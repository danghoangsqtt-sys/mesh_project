...
export const useWebSocket = (url: string = 'ws://localhost:8000/ws') => {
  ...
  const { updateNode, addEvent, setConnectionStatus, setMapDownloadState, incrementMapVersion, addRawLog } = useMeshStore();
    ...
    const connect = () => {
      ...
      ws.current.onmessage = (event) => {
        try {
          ...
          } else if (data.type === 'map_download_progress') {
            setMapDownloadState(data.data.progress, data.data.status);
          } else if (data.type === 'map_ready') {
            incrementMapVersion();
          } else if (data.type === 'raw_serial') {
            addRawLog(data.data);
          }
        } catch (err) {
